# data-update-records Specification

## Purpose

Define the frontend navigation, querying, manual submission, filtering, pagination, and result rendering behavior for daily dashboard data update records.
...
Supported data types SHALL include `core_etf`, `market_core`, `zhongxin_future`, and `dividend_low_vol`.
Supported trigger sources SHALL include `api`, `scheduler`, and `repair_scheduler`.
...
- **WHEN** the user selects API, scheduler, or missing-data repair as the trigger source and searches
...
#### Scenario: Render a missing-data repair run

- **WHEN** a run has `trigger_source=repair_scheduler`
- **THEN** the table MUST render the trigger source as an automatic missing-data repair source
- **AND** the trigger-source filter MUST allow selecting only missing-data repair runs
...
Supported manual-submission data types SHALL include `core_etf`, `market_core`, `zhongxin_future`, and `dividend_low_vol`.
